...
public enum OctahedralGroup implements StringRepresentable {
    ...
    private static final OctahedralGroup[][] cayleyTable = Util.make(new OctahedralGroup[values().length][values().length], (octahedralGroups) -> {
        Map<Pair<SymmetricGroup3, BooleanList>, OctahedralGroup> map = Arrays.stream(values()).collect(Collectors.toMap((octahedralGroupx) -> {
    ...
    private static final OctahedralGroup[] inverseTable = Arrays.stream(values()).map((octahedralGroup) -> {
        return Arrays.stream(values()).filter((octahedralGroup2) -> {
            return octahedralGroup.compose(octahedralGroup2) == IDENTITY;
        }).findAny().get();
    }).toArray((i) -> {
        return new OctahedralGroup[i];
    });

    private OctahedralGroup(String name, SymmetricGroup3 axisTransformation, boolean flipX, boolean flipY, boolean flipZ) {
        this.name = name;
        this.invertX = flipX;
        this.invertY = flipY;
        this.invertZ = flipZ;
    ...
    public OctahedralGroup compose(OctahedralGroup transformation) {
    ...
    public Direction rotate(Direction direction) {
        if (this.rotatedDirections == null) {
            ...
            for(Direction direction2 : Direction.values()) {
                ...
                Direction.Axis axis2 = Direction.Axis.values()[this.permutation.permutation(axis.ordinal())];
