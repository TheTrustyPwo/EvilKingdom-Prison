...
public class Noises {
    ...
    private static ResourceKey<NormalNoise.NoiseParameters> createKey(String id) {
        return ResourceKey.create(Registry.NOISE_REGISTRY, new ResourceLocation(id));
    }

    public static NormalNoise instantiate(Registry<NormalNoise.NoiseParameters> noiseRegistry, PositionalRandomFactory randomDeriver, ResourceKey<NormalNoise.NoiseParameters> noise) {
        Holder<NormalNoise.NoiseParameters> holder = noiseRegistry.getHolderOrThrow(noise);
        return instantiate(randomDeriver, holder);
    }
