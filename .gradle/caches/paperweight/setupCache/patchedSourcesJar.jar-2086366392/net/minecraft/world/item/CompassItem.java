...
public class CompassItem extends Item implements Vanishable {
    ...
    public static Optional<ResourceKey<Level>> getLodestoneDimension(CompoundTag nbt) {
        ...
        if (!world.isClientSide) {
            if (isLodestoneCompass(stack)) {
                ...
                if (optional.isPresent() && optional.get() == world.dimension() && compoundTag.contains("LodestonePos")) {
                    ...
                    if (!world.isInWorldBounds(blockPos) || !((ServerLevel)world).getPoiManager().existsAtPosition(PoiType.LODESTONE, blockPos)) {
