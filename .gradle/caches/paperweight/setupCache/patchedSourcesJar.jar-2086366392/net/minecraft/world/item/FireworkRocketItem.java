...
public class FireworkRocketItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag context) {
        ...
        if (compoundTag != null) {
            if (compoundTag.contains("Flight", 99)) {
                tooltip.add((new TranslatableComponent("item.minecraft.firework_rocket.flight")).append(" ").append(String.valueOf((int)compoundTag.getByte("Flight"))).withStyle(ChatFormatting.GRAY));
            }
    ...
    public static enum Shape {
        ...
        private static final FireworkRocketItem.Shape[] BY_ID = Arrays.stream(values()).sorted(Comparator.comparingInt((type) -> {
            return type.id;
        })).toArray((i) -> {
