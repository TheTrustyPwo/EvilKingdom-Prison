...
public class GolemRandomStrollInVillageGoal extends RandomStrollGoal {
    ...
    public GolemRandomStrollInVillageGoal(PathfinderMob entity, double speed) {
    ...
    @Nullable
    private BlockPos getRandomPoiWithinSection(SectionPos pos) {
        ServerLevel serverLevel = (ServerLevel)this.mob.level;
        PoiManager poiManager = serverLevel.getPoiManager();
        List<BlockPos> list = poiManager.getInRange((poiType) -> {
            return true;
        }, pos.center(), 8, PoiManager.Occupancy.IS_OCCUPIED).map(PoiRecord::getPos).collect(Collectors.toList());
        return list.isEmpty() ? null : list.get(serverLevel.random.nextInt(list.size()));
    }
