...
public class ChunkHeightAndBiomeFix extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ChunkHeightAndBiomeFix", type, type2, (typed) -> {
            return typed.updateTyped(opticFinder, type3, (typed2) -> {
                ...
                typed2 = typed2.updateTyped(opticFinder2, type4, (typedx) -> {
                    ...
                    Dynamic<?> dynamic3 = typedx.write().result().orElseThrow(() -> {
                    ...
                    List<Dynamic<?>> list = dynamic3.asStream().map((dynamic2x) -> {
                        ...
                        Dynamic<?> dynamic3 = DataFixUtils.orElse(dynamic2x.get("Palette").result().flatMap((dynamic2xx) -> {
                            dynamic2xx.asStream().map((dynamic) -> {
                                return dynamic.get("Name").asString("minecraft:air");
                            }).forEach(set::add);
                            return dynamic2x.get("BlockStates").result().map((dynamic2) -> {
                                return makeOptimizedPalettedContainer(dynamic2xx, dynamic2);
                            });
                        }), dynamic2);
                        Dynamic<?> dynamic4 = dynamic2x;
                        int k = j - i;
                        if (k >= 0 && k < dynamics.length) {
                            dynamic4 = dynamic2x.set("biomes", dynamics[k]);
                        }
                    ...
                    return type4.readTyped(dynamic.createList(list.stream())).result().orElseThrow(() -> {
    ...
    private static Dynamic<?>[] getBiomeContainers(Dynamic<?> dynamic, boolean bl, int i, MutableBoolean mutableBoolean) {
        ...
        int[] is = dynamic.get("Biomes").asIntStreamOpt().result().map(IntStream::toArray).orElse((int[])null);
        if (is != null && is.length == 1536) {
            mutableBoolean.setValue(true);

            for(int j = 0; j < 24; ++j) {
                dynamics[j] = makeBiomeContainer(dynamic, (j) -> {
                    return getOldBiome(is, j * 64 + j);
                });
            }
        } else if (is != null && is.length == 1024) {
            for(int l = 0; l < 16; ++l) {
                int m = l - i;
                dynamics[m] = makeBiomeContainer(dynamic, (j) -> {
                    return getOldBiome(is, l * 64 + j);
                });
            }

            if (bl) {
                Dynamic<?> dynamic2 = makeBiomeContainer(dynamic, (ix) -> {
                    return getOldBiome(is, ix % 16);
                });
                Dynamic<?> dynamic3 = makeBiomeContainer(dynamic, (ix) -> {
                    return getOldBiome(is, ix % 16 + 1008);
                });
    ...
    private static <T> Dynamic<T> shiftUpgradeData(Dynamic<T> dynamic) {
        return dynamic.update("Indices", (dynamicx) -> {
            ...
            dynamicx.getMapValues().result().ifPresent((map2) -> {
    ...
    private static Dynamic<?> updateCarvingMasks(Dynamic<?> dynamic, int i, int j) {
        ...
        dynamic2 = dynamic2.updateMapValues((pair) -> {
            long[] ls = BitSet.valueOf(pair.getSecond().asByteBuffer().array()).toLongArray();
            long[] ms = new long[64 * i];
            System.arraycopy(ls, 0, ms, 64 * j, ls.length);
            return Pair.of(pair.getFirst(), dynamic.createLongList(LongStream.of(ms)));
        });
        return dynamic.set("CarvingMasks", dynamic2);
    }

    private static Dynamic<?> addPaddingEntries(Dynamic<?> dynamic, String string) {
        List<Dynamic<?>> list = dynamic.get(string).orElseEmptyList().asStream().collect(Collectors.toCollection(ArrayList::new));
        if (list.size() == 24) {
            return dynamic;
        } else {
    ...
    private static Dynamic<?> makeBiomeContainer(Dynamic<?> dynamic, Int2IntFunction int2IntFunction) {
        ...
        Dynamic<?> dynamic2 = dynamic.createList(int2IntMap.keySet().stream().map((integer) -> {
            return dynamic.createString(BIOMES_BY_ID.getOrDefault(integer.intValue(), "minecraft:plains"));
        }));
    ...
    public static int ceillog2(int value) {
        return value == 0 ? 0 : (int)Math.ceil(Math.log((double)value) / Math.log(2.0D));
    }
