...
public class EntityBlockStateFix extends DataFix {
    ...
    public static int getBlockId(String blockId) {
        Integer integer = MAP.get(blockId);
    ...
    public TypeRewriteRule makeRule() {
        ...
        Function<Typed<?>, Typed<?>> function3 = (typed) -> {
        ...
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(References.ENTITY), schema2.getType(References.ENTITY), (typed) -> {
            ...
            typed = this.updateEntity(typed, "minecraft:enderman", (typedx) -> {
                return this.updateBlockToBlockState(typedx, "carried", "carriedData", "carriedBlockState");
            });
    ...
    private Typed<?> updateFallingBlock(Typed<?> typed) {
        ...
        return typed.update(type.finder(), type2, (either) -> {
            int i = either.map((pair) -> {
                return pair.getSecond().map((integer) -> {
                    return integer;
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> typed, String oldIdKey, String oldDataKey, String newStateKey) {
        Type<Pair<String, Either<Integer, String>>> type = DSL.field(oldIdKey, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())));
        ...
        return typed.update(type.finder(), type2, (pair) -> {
            int i = pair.getSecond().map((integer) -> {
                return integer;
            }, EntityBlockStateFix::getBlockId);
    ...
    private Typed<?> updateEntity(Typed<?> typed, String entityId, Function<Typed<?>, Typed<?>> function) {
