...
public class RecipeCommand {
    ...
    private static int giveRecipes(CommandSourceStack source, Collection<ServerPlayer> targets, Collection<Recipe<?>> recipes) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            if (targets.size() == 1) {
                source.sendSuccess(new TranslatableComponent("commands.recipe.give.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
            } else {
                source.sendSuccess(new TranslatableComponent("commands.recipe.give.success.multiple", recipes.size(), targets.size()), true);
            }
    ...
    private static int takeRecipes(CommandSourceStack source, Collection<ServerPlayer> targets, Collection<Recipe<?>> recipes) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            if (targets.size() == 1) {
                source.sendSuccess(new TranslatableComponent("commands.recipe.take.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
            } else {
                source.sendSuccess(new TranslatableComponent("commands.recipe.take.success.multiple", recipes.size(), targets.size()), true);
            }
