...
public class ObjectiveCriteria {
    ...
    protected ObjectiveCriteria(String name, boolean readOnly, ObjectiveCriteria.RenderType defaultRenderType) {
        this.name = name;
    ...
    public static Optional<ObjectiveCriteria> byName(String name) {
        ...
        if (objectiveCriteria != null) {
            ...
            return i < 0 ? Optional.empty() : Registry.STAT_TYPE.getOptional(ResourceLocation.of(name.substring(0, i), '.')).flatMap((type) -> {
                return getStat(type, ResourceLocation.of(name.substring(i + 1), '.'));
            });
        }
    }

    private static <T> Optional<ObjectiveCriteria> getStat(StatType<T> statType, ResourceLocation id) {
    ...
    public static enum RenderType {
        ...
        public static ObjectiveCriteria.RenderType byId(String name) {
