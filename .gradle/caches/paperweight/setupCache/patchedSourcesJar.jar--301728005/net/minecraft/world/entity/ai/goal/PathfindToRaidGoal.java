...
public class PathfindToRaidGoal<T extends Raider> extends Goal {
    ...
    public PathfindToRaidGoal(T actor) {
        this.mob = actor;
    ...
    private void recruitNearby(Raid raid) {
        if (raid.isActive()) {
            ...
            List<Raider> list = this.mob.level.getEntitiesOfClass(Raider.class, this.mob.getBoundingBox().inflate(16.0D), (raiderx) -> {
                return !raiderx.hasActiveRaid() && Raids.canJoinRaid(raiderx, raid);
            });
            set.addAll(list);

            for(Raider raider : set) {
                raid.joinRaid(raid.getGroupsSpawned(), raider, (BlockPos)null, true);
            }
        }
