...
public class SpawnPlacements {
    ...
    private static <T extends Mob> void register(EntityType<T> type, SpawnPlacements.Type location, Heightmap.Types heightmapType, SpawnPlacements.SpawnPredicate<T> predicate) {
        SpawnPlacements.Data data = DATA_BY_TYPE.put(type, new SpawnPlacements.Data(heightmapType, location, predicate));
        if (data != null) {
            throw new IllegalStateException("Duplicate registration for type " + Registry.ENTITY_TYPE.getKey(type));
        }
    }

    public static SpawnPlacements.Type getPlacementType(EntityType<?> type) {
    ...
    public static <T extends Entity> boolean checkSpawnRules(EntityType<T> type, ServerLevelAccessor world, MobSpawnType spawnReason, BlockPos pos, Random random) {
        SpawnPlacements.Data data = DATA_BY_TYPE.get(type);
    ...
    static class Data {
        ...
        public Data(Heightmap.Types heightmapType, SpawnPlacements.Type location, SpawnPlacements.SpawnPredicate<?> predicate) {
    ...
    public interface SpawnPredicate<T extends Entity> {
        boolean test(EntityType<T> type, ServerLevelAccessor world, MobSpawnType spawnReason, BlockPos pos, Random random);
    }
