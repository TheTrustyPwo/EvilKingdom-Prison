...
public final class LevelStem {
    ...
    public static Registry<LevelStem> sortMap(Registry<LevelStem> registry) {
        ...
        for(Entry<ResourceKey<LevelStem>, LevelStem> entry : registry.entrySet()) {
            ...
            if (!BUILTIN_ORDER.contains(resourceKey2)) {
                writableRegistry.register(resourceKey2, entry.getValue(), registry.lifecycle(entry.getValue()));
            }
        }
    ...
    public static boolean stable(long seed, Registry<LevelStem> registry) {
        if (registry.size() != BUILTIN_ORDER.size()) {
            ...
            if (!optional.isEmpty() && !optional2.isEmpty() && !optional3.isEmpty()) {
                if (!optional.get().typeHolder().is(DimensionType.OVERWORLD_LOCATION) && !optional.get().typeHolder().is(DimensionType.OVERWORLD_CAVES_LOCATION)) {
                    return false;
                } else if (!optional2.get().typeHolder().is(DimensionType.NETHER_LOCATION)) {
                    return false;
                } else if (!optional3.get().typeHolder().is(DimensionType.END_LOCATION)) {
                    return false;
                    ...
                    if (!noiseBasedChunkGenerator.stable(seed, NoiseGeneratorSettings.NETHER)) {
                        return false;
                    } else if (!noiseBasedChunkGenerator2.stable(seed, NoiseGeneratorSettings.END)) {
                        return false;
