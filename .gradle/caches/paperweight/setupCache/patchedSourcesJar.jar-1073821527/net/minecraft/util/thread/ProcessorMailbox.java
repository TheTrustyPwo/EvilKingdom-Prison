...
public class ProcessorMailbox<T> implements ProfilerMeasured, ProcessorHandle<T>, AutoCloseable, Runnable {
    ...
    public static ProcessorMailbox<Runnable> create(Executor executor, String name) {
        return new ProcessorMailbox<>(new StrictQueue.QueueStrictQueue<>(new ConcurrentLinkedQueue<>()), executor, name);
    }

    public ProcessorMailbox(StrictQueue<? super T, ? extends Runnable> queue, Executor executor, String name) {
