...
public class ChunkProtoTickListFix extends DataFix {
        ...
        return this.fixTypeEverywhereTyped("ChunkProtoTickListFix", type, (typed) -> {
            return typed.updateTyped(opticFinder, (typedx) -> {
                typedx = typedx.update(DSL.remainderFinder(), (dynamicx) -> {
                    return DataFixUtils.orElse(dynamicx.get("LiquidTicks").result().map((dynamic2) -> {
                ...
                if (!typedx.getOptionalTyped(opticFinder7).isPresent() && !dynamic.get("fluid_ticks").result().isPresent()) {
                    ...
                    if (optional.isPresent()) {
                        typedx = typedx.set(opticFinder7, optional.get().getFirst());
                    }
    ...
    private Dynamic<?> makeTickList(Dynamic<?> dynamic, Int2ObjectMap<Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer>> int2ObjectMap, byte b, int i, int j, String string, Function<Dynamic<?>, String> function) {
        ...
        for(int k = 0; k < list.size(); ++k) {
            ...
            Stream<? extends Dynamic<?>> stream2 = list.get(k).asStream().mapToInt((dynamicx) -> {
                return dynamicx.asShort((short)-1);
            }).filter((ix) -> {
                return ix > 0;
            }).mapToObj((lx) -> {
                return this.createTick(dynamic, supplier, i, l, j, lx, function);
            });
        ...
        public Dynamic<?> get(int i, int j, int k) {
            ...
            if (l < 1) {
                ...
                if (n >= 0 && n < this.data.length) {
                    ...
                    return q >= 0 && q < l ? this.palette.get(q) : null;
                } else {
                    return null;
                }
            }
