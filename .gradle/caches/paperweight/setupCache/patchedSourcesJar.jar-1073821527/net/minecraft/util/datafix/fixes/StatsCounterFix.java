    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", this.getInputSchema().getType(References.STATS), type, (typed) -> {
            ...
            if (optional.isPresent()) {
                Iterator var6 = optional.get().entrySet().iterator();
                ...
                while(true) {
                            ...
                            if (!SKIP.contains(string)) {
                                ...
                                if (i >= 0) {
                                    ...
                                    if (ENTITY_KEYS.containsKey(string4)) {
                                        ...
                                        string3 = ENTITIES.getOrDefault(string12, string12);
                                        break;
                                    }
                                }
                            }
                    ...
                    Dynamic<?> dynamic3 = map.computeIfAbsent(dynamic2, (dynamic2x) -> {
                        return dynamic.emptyMap();
                    });
                    map.put(dynamic2, dynamic3.set(string3, entry.getValue()));
                }
            } else {
                return type.readTyped(dynamic.emptyMap().set("stats", dynamic.createMap(map))).result().orElseThrow(() -> {
                    return new IllegalStateException("Could not parse new stats object.");
