...
public class JigsawRotationFix extends DataFix {
    ...
    private static Dynamic<?> fix(Dynamic<?> dynamic) {
        ...
        return optional.equals(Optional.of("minecraft:jigsaw")) ? dynamic.update("Properties", (dynamicx) -> {
            ...
            return dynamicx.remove("facing").set("orientation", dynamicx.createString(RENAMES.getOrDefault(string, string)));
        }) : dynamic;
    }

    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("jigsaw_rotation_fix", this.getInputSchema().getType(References.BLOCK_STATE), (typed) -> {
            return typed.update(DSL.remainderFinder(), JigsawRotationFix::fix);
        });
    }
}
