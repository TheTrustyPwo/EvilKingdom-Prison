...
public class ItemModelGenerators {
    ...
    public ItemModelGenerators(BiConsumer<ResourceLocation, Supplier<JsonElement>> writer) {
        this.output = writer;
    }

    private void generateFlatItem(Item item, ModelTemplate model) {
        model.create(ModelLocationUtils.getModelLocation(item), TextureMapping.layer0(item), this.output);
    }

    private void generateFlatItem(Item item, String suffix, ModelTemplate model) {
        model.create(ModelLocationUtils.getModelLocation(item, suffix), TextureMapping.layer0(TextureMapping.getItemTexture(item, suffix)), this.output);
    }

    private void generateFlatItem(Item item, Item texture, ModelTemplate model) {
        model.create(ModelLocationUtils.getModelLocation(item), TextureMapping.layer0(texture), this.output);
    }
