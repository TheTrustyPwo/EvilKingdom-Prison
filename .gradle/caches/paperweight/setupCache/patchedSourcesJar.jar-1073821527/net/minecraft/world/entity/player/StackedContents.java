...
public class StackedContents {
    ...
    public int getBiggestCraftableStack(Recipe<?> recipe, @Nullable IntList output) {
        return this.getBiggestCraftableStack(recipe, Integer.MAX_VALUE, output);
    }

    public int getBiggestCraftableStack(Recipe<?> recipe, int limit, @Nullable IntList output) {
        return (new StackedContents.RecipePicker(recipe)).tryPickAll(limit, output);
    }
        ...
        public boolean tryPick(int multiplier, @Nullable IntList output) {
            if (multiplier <= 0) {
                ...
                for(int m = 0; m < list.size(); ++m) {
                    if (bl2 && list.get(m).isEmpty()) {
                        output.add(0);
                    } else {
                        for(int n = 0; n < this.itemCount; ++n) {
                            if (this.hasResidual(false, l, n)) {
                                this.toggleResidual(true, n, l);
                                StackedContents.this.put(this.items[n], multiplier);
                                if (bl2) {
                                    output.add(this.items[n]);
                                }
                            }
                        }
