...
public class ClearInventoryCommands {
    private static final DynamicCommandExceptionType ERROR_SINGLE = new DynamicCommandExceptionType((playerName) -> {
        return new TranslatableComponent("clear.failed.single", playerName);
    });
    private static final DynamicCommandExceptionType ERROR_MULTIPLE = new DynamicCommandExceptionType((playerCount) -> {
        return new TranslatableComponent("clear.failed.multiple", playerCount);
    });
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        }).then(Commands.argument("item", ItemPredicateArgument.itemPredicate()).executes((context) -> {
    ...
    private static int clearInventory(CommandSourceStack source, Collection<ServerPlayer> targets, Predicate<ItemStack> item, int maxCount) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            if (maxCount == 0) {
                if (targets.size() == 1) {
                    source.sendSuccess(new TranslatableComponent("commands.clear.test.single", i, targets.iterator().next().getDisplayName()), true);
                } else {
                    source.sendSuccess(new TranslatableComponent("commands.clear.test.multiple", i, targets.size()), true);
                }
            } else if (targets.size() == 1) {
                source.sendSuccess(new TranslatableComponent("commands.clear.success.single", i, targets.iterator().next().getDisplayName()), true);
            } else {
                source.sendSuccess(new TranslatableComponent("commands.clear.success.multiple", i, targets.size()), true);
            }
